...
@router.post("/create-user", status_code=200, dependencies=[Depends(get_db)])
def create_user(user_data: dict = Body(...), Authorize: AuthJWT = Depends()):
    return user_ops.create_user(user_data, Authorize)


@router.get("/user-info", status_code=200, dependencies=[Depends(get_db)])
def get_user_details(email: str):
    return user_ops.get_user_info(email)


@router.get("/user-info-id", status_code=200, dependencies=[Depends(get_db)])
def get_user_details(id: str):
    return user_ops.get_user_info_by_id(id)


@router.get("/search-user", status_code=200, dependencies=[Depends(get_db)])
def get_user_match(user_info: str):
    return user_ops.search_user(user_info)


@router.post("/user-update-passwords", status_code=200, dependencies=[Depends(get_db)])
